...
extern class Particles extends godot.GeometryInstance {
	...
	**/
	...
	/**		
		The `godot.AABB` that determines the node's region which needs to be visible on screen for the particle system to be active.
		
		Grow the box if particles suddenly appear/disappear when the node enters/exits the screen. The `godot.AABB` can be grown via code or with the Particles → Generate AABB editor tool.
	...
	**/
	...
	/**		
		The number of particles emitted in one emission cycle (corresponding to the `godot.Particles.lifetime`).
		
		Note: Changing `godot.Particles.amount` will reset the particle emission, therefore removing all particles that were already emitted before changing `godot.Particles.amount`.
	**/
	@:native("Amount")
